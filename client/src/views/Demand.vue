...
export default {
  ...
  setup() {
    ...
    const forecastsByTrend = computed(() => ({
      increasing: forecasts.value.filter(f => f.trend === 'increasing'),
      stable: forecasts.value.filter(f => f.trend === 'stable'),
      decreasing: forecasts.value.filter(f => f.trend === 'decreasing')
    }))
